...
There are other differences.  In some cases this package is more
...
the built-in functions only support pre-releases of sorts but not
patches (~-pN~) like this library does.
...
lengths, ~vcomp--compare-interned~ fills in ~0~.
